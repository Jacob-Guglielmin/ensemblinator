...
class NotifierErrorHandler(logging.Handler):
    def emit(self, record):
        ...
        try:
            ...
            logging.getLogger("ensemblinator").warning("notifier not yet initialized: failed to deliver error notification")
        except Exception:
            logging.getLogger("ensemblinator").warning("failed to deliver error notification")

def configure_logging(level=logging.INFO):
    stream_handler = logging.StreamHandler(sys.stderr)
    stream_handler.setFormatter(StreamFormatter("%(asctime)s [%(levelname)s] [ensemblinator]: %(message)s", datefmt="%Y-%m-%dT%H:%M:%SZ"))
